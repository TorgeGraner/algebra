...
* TODO: Improve factorization of polynomial
*/
template <typename R>
Matrix<R> decompose(Matrix<R>& A) {
	...
	while (getFactor(cPoly, currFactor)) {
		...
		int alg_mult = 0;
		while (cPoly.map(eig) == 0) { // Not safe
			cPoly /= *currFactor;
			++alg_mult;
		}
		...
		// Calculate the complements ker psi^(k+1) / ker psi^k
		...
		int len = complements.size();
		for (int i = len - 1; i >= 0; --i) {
			// Add all jordan chains of remaining vectors
			while (complements[i].getM() != 0) { // Not safe
