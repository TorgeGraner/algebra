...
Even though the native type int can be used as a template parameter, under given circumstances this can produce unwanted problems. Use the wrapper class Integer instead. 

The algorithm does not work well with floating point arithmetic, even with a wrapper class, since rounding error can still influence the rank of a matrix, leading to faulty logic producing errors. Some array is allocated somewhere that is never deallocated.
